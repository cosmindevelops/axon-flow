...
class ComputeService {
  ...
  getAverageTime(): number {
    return this.results.length > 0 ? this.results.reduce((a, b) => a + b, 0) / this.results.length : 0;
  }
}
...
describe("Cross-Platform Compatibility Integration", () => {
  ...
  describe("Node.js Version Compatibility", () => {
    ...
    it("should handle ES modules correctly", async () => {
      ...
      const hasESModules = PlatformDetector.isNode() && process.version.match(/^v(1[4-9]|[2-9]\d)\./); // Node 14+
  ...
  describe("Factory Pattern Cross-Platform", () => {
    it("should work consistently across platforms", () => {
      const factory = new SimpleFactory("TimestampFactory", () => {
    ...
    it("should handle async factory creation across platforms", async () => {
      const asyncFactory = new SimpleFactory("AsyncTimestampFactory", async () => {
  ...
  describe("Object Pool Cross-Platform", () => {
    it("should work consistently across platforms", async () => {
      const pool = new ObjectPool(COMPUTE_TOKEN, () => new ComputeService(), {
        minSize: 2,
        maxSize: 5,
        enableMetrics: true,
        evictionPolicy: "LRU",
