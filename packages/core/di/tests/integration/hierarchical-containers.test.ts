...
describe("Hierarchical Containers Integration", () => {
  ...
  describe("Basic Hierarchy Setup", () => {
    ...
    it("should allow registration at different levels", () => {
      ...
      requestContainer.registerFactory(REQUEST_CONTEXT_TOKEN, () => new RequestContextService(`req_${Date.now()}`));
  ...
  describe("Cross-Scope Dependencies", () => {
    ...
    it("should handle complex dependency graphs across scopes", () => {
      ...
      appContainer.registerFactory(
        APPLICATION_TOKEN,
        () =>
          new ApplicationService("ComplexApp", appContainer.resolve(DATABASE_TOKEN), appContainer.resolve(CACHE_TOKEN)),
      );

      // Register scoped service that depends on services from multiple levels
      requestContainer.registerFactory(
  ...
  describe("Scope Isolation and Override", () => {
    ...
    it("should isolate transient services between scopes", () => {
      ...
      featureContainer.registerFactory(REQUEST_CONTEXT_TOKEN, () => new RequestContextService("feature-context"));

      requestContainer.registerFactory(REQUEST_CONTEXT_TOKEN, () => new RequestContextService("request-context"));
