...
describe('Container Schemas', () => {
  ...
  describe('CONTAINER_METRICS_SCHEMA', () => {
    ...
    it('should reject negative integer values', () => {
      ...
      expect(() => CONTAINER_METRICS_SCHEMA.parse({ ...validMetrics(), ...negativeRegistrations })).toThrow('Total registrations cannot be negative');
      expect(() => CONTAINER_METRICS_SCHEMA.parse({ ...validMetrics(), ...negativeResolutions })).toThrow('Total resolutions cannot be negative');
    });
    ...
    it('should reject negative time values', () => {
      ...
      expect(() => CONTAINER_METRICS_SCHEMA.parse({ ...validMetrics(), ...negativeAverage })).toThrow('Average resolution time cannot be negative');
      expect(() => CONTAINER_METRICS_SCHEMA.parse({ ...validMetrics(), ...negativePeak })).toThrow('Peak resolution time cannot be negative');
    });
    ...
    it('should reject invalid cache hit ratio values', () => {
      ...
      expect(() => CONTAINER_METRICS_SCHEMA.parse({ ...validMetrics(), ...negativeCacheRatio })).toThrow('Cache hit ratio cannot be negative');
      expect(() => CONTAINER_METRICS_SCHEMA.parse({ ...validMetrics(), ...tooLargeCacheRatio })).toThrow('Cache hit ratio cannot exceed 1');
    });
    ...
    it('should reject negative memory usage values', () => {
      ...
      expect(() => CONTAINER_METRICS_SCHEMA.parse({ ...validMetrics(), ...invalidMemoryUsage })).toThrow('Singleton count cannot be negative');
    });
