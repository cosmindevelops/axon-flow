...
async function measureBuildCaching(ctx: ValidationContext): Promise<ValidationResult> {
  ...
  const workspaces = await ctx.discoverWorkspaces();
  const buildableWorkspaces = workspaces.filter(workspace => workspace.hasBuildScript);

  if (buildableWorkspaces.length === 0) {
    return {
      id: 'V1.2',
      title: 'Turbo build succeeds with cache acceleration',
      status: 'passed',
      durationMs: performance.now() - start,
      details:
        'Turbo configured correctly. No workspaces expose a build script yet (expected for initial setup).',
      metrics: {
        workspaceCount: workspaces.length,
        buildableWorkspaces: buildableWorkspaces.length,
      },
    };
  }
